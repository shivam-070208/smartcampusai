...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchIssues = async (currentUser: typeof user) => {
      try {
        if (!currentUser) {
          return;
        }
  ...
  const processedIssues = issues
    .filter((issue) => {
      ...
      if (filter === "active")
        return issue.status === "active" || issue.status === "pending";
        ...
        <div className="p-4 border-b border-gray-200 flex justify-between items-center">
          <div className="flex gap-2">
            <span className="text-sm font-medium text-gray-500 self-center mr-2">
              Showing:{" "}
              <span className="text-black font-bold capitalize">{filter}</span>
            </span>
          </div>
